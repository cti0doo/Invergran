"""
Upgrade account_loan to allow months and years as loan_term_unit & payment schedule up to balance <= 0
"""
...
class Loan(object):
    ...
    def __init__(self, loan_amount, interest_rate, loan_term, loan_term_unit, start_date,
                payment_amount=None, first_payment_date=None, payment_end_of_month=True, annual_payments=12,
        ...
        try:
            ...
            if self.loan_term_unit == 'year':
                ...
                self.no_of_payments = self.loan_term
    ...
    def _get_special_payment_schedule(self, special_payment):
        ...
        while special_payment.loan_balance_amount > 0:
    ...
    def get_payment_schedule(self):
        ...
        try:
            ...
            while payment.loan_balance_amount >= 1:
                i += 1
                self.no_of_payments = self.no_of_payments + 1
                date = dt0 + relativedelta(months=i * 12 / self.annual_payments)
                if self.payment_end_of_month == True and self.first_payment_date is None:
                    eom_day = cal.monthrange(date.year, date.month)[1]
                    date = date.replace(day=eom_day)  # dt.datetime(date.year,date.month,eom_day)
